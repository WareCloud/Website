...
\IPS\Content\Embeddable
{
	...
	public function customFields( $topic = FALSE )
	{
		...
		if( $topic === TRUE )
		{
			$fieldData	= iterator_to_array( \IPS\Db::i()->select( 'cf_id,cf_format', 'downloads_cfields', array( 'cf_topic=?', 1 ) )->setKeyField( 'cf_id' )->setValueField( 'cf_format' ) );
		}
	 ...
	 */
	public function apiOutput( $backup=NULL )
	{		
		return array(
			'id'				=> $this->tid,
			...
			'files'				=> array_values( array_map( function( $file ) {
				return $file->apiOutput();
			}, iterator_to_array( $this->files( $backup ? $backup['b_id'] : NULL ) ) ) ),
			'screenshots'		=> array_values( array_map( function( $file ) {
				return $file->apiOutput();
			}, iterator_to_array( $this->screenshots( 0, TRUE, $backup ? $backup['b_id'] : NULL ) ) ) ),
			'primaryScreenshot'	=> $this->primary_screenshot ? ( $this->primary_screenshot->apiOutput() ) : null,
