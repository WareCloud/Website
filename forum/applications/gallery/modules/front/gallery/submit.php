...
class _submit extends \IPS\Dispatcher\Controller
{
	...
	protected function manage()
	{
		...
		{
			$initialData['category'] = \IPS\Request::i()->category;
		}
		...
		), \IPS\Http\Url::internal( 'app=gallery&module=gallery&controller=submit', 'front', 'gallery_submit' ), TRUE, $initialData );
	...
	public function _stepUploadImages( $data )
	{
		...
		/* Init form */
		$form = new \IPS\Helpers\Form( 'upload_images', 'continue', \IPS\Http\Url::internal( 'app=gallery&module=gallery&controller=submit&_step=upload_images', 'front', 'gallery_submit' ) );
	...
	public function _stepAddInfo( $data )
	{
		...
		$allImagesForm = new \IPS\Helpers\Form( 'all_images_form', 'submit', \IPS\Http\Url::internal( 'app=gallery&module=gallery&controller=submit&_step=image_information', 'front', 'gallery_submit' ) );
	...
	protected function _editImageForm( $images, $currentlyEditing, $category )
	{
		...
		/* Initate form */
		$form	= new \IPS\Helpers\Form( 'image_information_' . $currentlyEditing, 'continue_next_image', \IPS\Http\Url::internal( 'app=gallery&module=gallery&controller=submit&_step=image_information', 'front', 'gallery_submit' ) );
		...
		foreach ( \IPS\gallery\Image::formElements( NULL, $category ) as $input )
		{
			...
			if ( !$input->value )
			{
				...
				{
					$input->value = str_replace( "_", "-", $images[ $currentlyEditing ]->originalFilename );
				}
			}
	...
	public function _stepProcess( $data )
	{
		...
		/* Process */
		$multiRedirect = (string) new \IPS\Helpers\MultipleRedirect( \IPS\Http\Url::internal( "app=gallery&module=gallery&controller=submit&_step=process", 'front', 'gallery_submit' ),

			/* Function to process each image */
			function( $offset ) use ( $data, $category, $album )
			{
				$existing = \IPS\Db::i()->select( '*', 'gallery_images_uploads', array( 'upload_session=?', session_id() ), 'upload_location', array( 0, 1 ) )->setKeyField( 'upload_location' );
				foreach( $existing as $location => $file )
				{
