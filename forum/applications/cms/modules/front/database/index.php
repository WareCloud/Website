...
class _index extends \IPS\cms\Databases\Controller
{
	...
	protected function view()
	{
		...
		if ( $database->cat_index_type === 1 and ! isset( \IPS\Request::i()->show ) )
		{
			...
			if ( isset( $database->featured_settings['categories'] ) and count( $database->featured_settings['categories'] ) AND is_array( $database->featured_settings['categories'] ) )
			{
				$where[] = array( \IPS\Db::i()->in( 'category_id', array_values( $database->featured_settings['categories'] ) ) );
			}
			
			$articles = $recordClass::getItemsWithPermission( $where, 'record_pinned DESC, ' . $database->featured_settings['sort'] . ' ' . $database->featured_settings['direction'], array( $limit, $database->featured_settings['perpage'] ), 'read', \IPS\Content\Hideable::FILTER_AUTOMATIC, 0, NULL, FALSE, FALSE, FALSE, FALSE );
			...
			\IPS\Output::i()->title = ( $page > 1 ) ? \IPS\Member::loggedIn()->language()->addToStack( 'title_with_page_number', FALSE, array( 'sprintf' => array( $database->_title, $page ) ) ) : $database->_title;
