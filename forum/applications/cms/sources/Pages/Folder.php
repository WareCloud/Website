...
class _Folder extends \IPS\Node\Model
{
	...
	{
		return (int) \IPS\Db::i()->select( 'COUNT(*)', 'cms_pages', array( 'page_folder_id=?', $this->id ) )->first();
	}
	...
	public function deleteOrMoveFormSubmit( $values, $deleteWhenDone=TRUE )
	{
		...
		{
			$folderId = $values['cms_move_pages']->id;
		}
		else
		{
			$folderId = 0;
		...
		\IPS\Db::i()->update( 'cms_pages', array( 'page_folder_id' => $folderId ), array( 'page_folder_id=?', $this->id ) );
		\IPS\Db::i()->update( 'cms_folders', array( 'folder_parent_id' => $folderId ), array( 'folder_id=?', $this->id ) );
