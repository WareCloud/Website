...
class _Categories extends \IPS\Node\Model implements \IPS\Node\Permissions
{
	 ...
	 */
	protected $_stripTagsCatTitle = NULL;
	...
	public function get__stripTagsTitle()
	{
		...
		if ( ! \IPS\cms\Databases::load( $this->database_id )->use_categories )
		{
			if ( ! $this->_stripTagsCatTitle )
			{
				try
				{
					$page = \IPS\cms\Pages\Page::loadByDatabaseId( $this->database_id );
					$this->_stripTagsCatTitle = $page->_stripTagsTitle;
				}
				catch( \OutOfRangeException $e )
				{
					$this->_stripTagsCatTitle = parent::get__stripTagsTitle();
				}
			}

			return $this->_stripTagsCatTitle;
		}
		else
		{
			return parent::get__stripTagsTitle();
		}
	}
