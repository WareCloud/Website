...
class _Parser
{
	...
	{	
		...
		/* Some tags have multiple __base_url__ replacements, so we have to replace this in a safe way ensuring we only match inside A, IMG, and IFRAME tags to prevent tampering */
		preg_match_all( '#<(img|a|iframe)([^>]+?)%7B___base_url___%7D([^>]+?)>#i', $value, $matches, PREG_SET_ORDER );
		foreach( $matches as $val )
		{
			$changed = preg_replace( '#(href|src|srcset|data\-fileid|data\-ipshover\-target)=(\'|")%7B___base_url___%7D/#i', '\1=\2<___base_url___>/', $val[0] );
			if ( $changed != $val[0] )
			{
